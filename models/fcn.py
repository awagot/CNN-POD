...
def model_fcn(channels, nz, nx, n_modes, cpu=False):
    ...
    model = layers.Conv2D(filters=int(512), kernel_size=(4, 4), activation=tf.nn.relu, data_format=data_format,padding = 'same')(inputs)
    model = layers.MaxPooling2D(pool_size=[2, 2],data_format=data_format,padding = 'same')(model) 

    for i in range(int(div_times-2)):
        model = layers.Conv2D(filters=int(512), kernel_size=(4, 4), activation=tf.nn.relu, data_format=data_format,padding = 'same')(model)
        model = layers.MaxPooling2D(pool_size=[2, 2],data_format=data_format,padding = 'same')(model) 

             
    model = layers.MaxPooling2D(pool_size=[2, 4],data_format=data_format,padding = 'same')(model)
